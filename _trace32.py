...
class T32:
    ...
    def get_symbol(symbol: str) -> tuple[int, int, int]:
        ...
        :return: (地址, 大小, 保留值)
        """
        address, size, reserved = c_uint32(), c_uint32(), c_uint32()
        err = __t32__.T32_GetSymbol(byref(sybol.))
        if error_mapping(err):
            raise error_mapping(err)()

    @staticmethod
    def get_symbol_from_address() -> int:
        """ """
        err = __t32__.T32_GetSymbolFromAddress()
        if error_mapping(err):
            raise error_mapping(err)()

    @staticmethod
    def read_variable_string() -> int:
        """ """
        err = __t32__.T32_ReadVariableString()
        if error_mapping(err):
            raise error_mapping(err)()

    @staticmethod
    def read_variable_value() -> int:
        """ """
        err = __t32__.T32_ReadVariableValue()
        if error_mapping(err):
            raise error_mapping(err)()

    @staticmethod
    def write_variable_value() -> int:
        """ """
        err = __t32__.T32_WriteVariableValue()
        if error_mapping(err):
            raise error_mapping(err)()

    @staticmethod
    def get_window_content() -> int:
        """ """
        err = __t32__.T32_GetWindowContent()
        if error_mapping(err):
            raise error_mapping(err)()
